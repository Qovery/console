...
**Never use ease-in for UI animations.** It starts slow — the exact moment the user is watching most closely — making the interface feel sluggish. A dropdown with `ease-in` at 300ms *feels* slower than `ease-out` at the same 300ms.
...
@media (prefers-reduced-motion: reduce) {
  .animate-slidein-up-sm-faded { animation: none; }
}
```
