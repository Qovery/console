...
- **Button with icon + text:** use `<Button>` and mark icons with `data-align="prefix"` or `data-align="suffix"` so the button primitive applies the correct size-aware spacing. Do not add manual icon margins or custom gaps inside buttons.
