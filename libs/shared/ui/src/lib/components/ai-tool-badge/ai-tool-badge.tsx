...
export const DEFAULT_AI_TOOL_NAME: AiToolName = AI_TOOL_NAMES[0]
...
const AI_TOOL_ICON_CLASSNAME = 'h-3.5 w-3.5'
...
const AI_BADGE_INTERVAL_MS = 4000
const AI_BADGE_ENTER_DELAY_SECONDS = 0.08
const AI_BADGE_ENTER_DURATION_SECONDS = 0.18
const AI_BADGE_REDUCED_MOTION_ENTER_DURATION_SECONDS = 0.16
const AI_BADGE_EXIT_DURATION_SECONDS = 0.14
const EASE_IN_QUAD = [0.55, 0.085, 0.68, 0.53] as const
const EASE_OUT_QUAD = [0.25, 0.46, 0.45, 0.94] as const

function badgeMotion(reducedMotion: boolean) {
  return {
    ...
    animate: reducedMotion
      ? {
          opacity: 1,
          transition: {
            delay: AI_BADGE_ENTER_DELAY_SECONDS,
            duration: AI_BADGE_REDUCED_MOTION_ENTER_DURATION_SECONDS,
            ease: EASE_OUT_QUAD,
          },
        }
      : {
          ...
          transition: {
            delay: AI_BADGE_ENTER_DELAY_SECONDS,
            duration: AI_BADGE_ENTER_DURATION_SECONDS,
            ease: EASE_IN_QUAD,
          },
        },
    exit: reducedMotion
      ? { opacity: 0, transition: { duration: AI_BADGE_EXIT_DURATION_SECONDS, ease: EASE_OUT_QUAD } }
      : {
          opacity: 0,
          filter: 'blur(4px)',
          transition: { duration: AI_BADGE_EXIT_DURATION_SECONDS, ease: EASE_OUT_QUAD },
        },
  }
}

function getAiToolName(aiName: string): AiToolName {
  return AI_TOOL_NAMES.includes(aiName as AiToolName) ? (aiName as AiToolName) : DEFAULT_AI_TOOL_NAME
}

function AiToolIcon({ tool }: { tool: AiToolBadgeConfig }) {
  if (tool.useCurrentColorIcon) {
    return (
      <span
        aria-hidden="true"
        className={`${AI_TOOL_ICON_CLASSNAME} bg-current`}
        style={{
  ...
  return <img src={tool.iconSrc} alt="" aria-hidden="true" className={AI_TOOL_ICON_CLASSNAME} />
}
...
export function AiToolBadge({ initialName = DEFAULT_AI_TOOL_NAME, animated = true }: AiToolBadgeProps) {
  const reducedMotion = useReducedMotion()
  const [toolIndex, setToolIndex] = useState(() => AI_TOOL_NAMES.indexOf(getAiToolName(initialName)))
  const tool = animated ? AI_TOOL_BADGES[AI_TOOL_NAMES[toolIndex]] : AI_TOOL_BADGES[getAiToolName(initialName)]
  const gradientStyle =
    tool.name === 'Codex'
  ...
  if (!animated) {
    return (
      <span className="inline-grid h-5 shrink-0 items-center">
        <span className={badgeClassName} style={tool.badgeStyle}>
