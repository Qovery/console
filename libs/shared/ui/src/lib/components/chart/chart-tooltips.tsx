...
export const ZoomRangeTooltip = forwardRef<HTMLDivElement, ZoomRangeTooltipProps>(function ZoomRangeTooltip(
  { active = true, startTime, endTime, useLocalTime = true },
  ref
) {
    ...
    <div ref={ref} className="rounded-md bg-neutral-600 shadow-lg">
      ...
      <div className="space-y-1 p-3 pt-0">
        <div className="flex items-center justify-between gap-4 text-xs">
          ...
          <span className="text-neutral-250">{formatTimestampForDisplay(start, useLocalTime)}</span>
        </div>
        <div className="flex items-center justify-between gap-4 text-xs">
          <span className="text-neutral-50">End:</span>
          <span className="text-neutral-250">{formatTimestampForDisplay(end, useLocalTime)}</span>
        </div>
      </div>
    </div>
  )
