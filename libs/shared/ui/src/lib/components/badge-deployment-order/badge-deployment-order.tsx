...
export function BadgeDeploymentOrder({ order = 0, className }: BadgeDeploymentOrderProps) {
  return (
    <div className={twMerge('relative flex drop-shadow-sm', className)}>
      ...
      <span className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 transform font-sans text-xs font-medium text-neutralInvert">
        {order}
      </span>
    </div>
  )
