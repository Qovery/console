...
export function StatusChip(props: StatusChipProps) {
  ...
  const showDeployed: boolean = [StateEnum.DEPLOYED, RunningState.RUNNING, RunningState.COMPLETED].includes(
    status as StateEnum
  )
  const showRestarted: boolean = [StateEnum.RESTARTED].includes(status as StateEnum)
  const showQueued: boolean = [StateEnum.QUEUED, StateEnum.DELETE_QUEUED, StateEnum.DEPLOYMENT_QUEUED].includes(
    status as StateEnum
  )
  const showBuilding: boolean = [StateEnum.BUILDING].includes(status as StateEnum)
  const showDeploying: boolean = [
    StateEnum.DEPLOYING,
    StateEnum.DELETING,
    StateEnum.RESTARTING,
    RunningState.STARTING,
  ].includes(status as StateEnum)
  ...
  return (
    <Tooltip content={tooltipContent}>
      <>
        {showDeployed && <DeployedIcon className={className} />}
        {showRestarted && <RestartedIcon className={className} />}
        {showQueued && <QueuedIcon className={className} />}
        {showBuilding && <BuildingIcon className={className} />}
        {showDeploying && <DeployingIcon className={className} />}
      </>
    </Tooltip>
  )
