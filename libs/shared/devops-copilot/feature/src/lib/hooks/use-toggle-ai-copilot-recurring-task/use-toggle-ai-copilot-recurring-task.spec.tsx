...
import { useToggleAICopilotRecurringTask } from './use-toggle-ai-copilot-recurring-task'
...
describe('useToggleAICopilotRecurringTask', () => {
  ...
  it('should call mutationFn with correct parameters', () => {
    ...
    renderHook(() => useToggleAICopilotRecurringTask({ organizationId }))

    expect(capturedMutationFn).toBeDefined()

    capturedMutationFn?.({ taskId: 'task-123' })
    expect(mutations.toggleRecurringTask).toHaveBeenCalledWith({
      organizationId,
      taskId: 'task-123',
    })
  })
  ...
  it('should invalidate queries and show success toast on success', () => {
    ...
    expect(toast).toHaveBeenCalledWith(ToastEnum.SUCCESS, 'Task status updated successfully')
  })
  ...
  it('should show error toast on error', () => {
    ...
    expect(toast).toHaveBeenCalledWith(ToastEnum.ERROR, 'Failed to update task status', 'Please try again later')
  })
})
