...
export function MenuAccount(props: MenuAccountProps) {
  ...
  const blockOrganization = (organization: OrganizationEntity) => (
    <div data-testid={`content-${organization.id}`} className="flex items-center">
      ...
      <span className="text-text-700 dark:text-zinc-100 text-sm font-medium">{organization.name}</span>
    </div>
  )
  ...
  const menus: MenuData = [
    ...
    },
    {
      items: [
        {
          itemContentCustom: (
            <div className="text-text-500 dark:text-zinc-100 text-ssm font-medium">
