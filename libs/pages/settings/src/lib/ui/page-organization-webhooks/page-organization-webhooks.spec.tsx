import { act, fireEvent, getAllByTestId, getByTestId, getByText } from '@testing-library/react'
...
describe('PageOrganizationWebhooks', () => {
  ...
  it('should display three rows with good values inside', async () => {
    ...
    const toggle = getByTestId(rows[0], 'input-toggle')
    await act(() => {
      toggle.click()
    })

    expect(props.onToggle).toHaveBeenCalledWith(mockWebhooks[0])
  })
