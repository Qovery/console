...
export function RowMember(props: RowMemberProps) {
  const { member, availableRoles, editMemberRole } = props
  ...
  const input = (role?: InviteMemberRoleEnum) => (
    <div
  ...
  return (
    <div className="grid grid-cols-4 border-b border-element-light-lighter-400 last:border-0">
      ...
      <div className="flex items-center px-4">
        {member.role !== InviteMemberRoleEnum.OWNER ? (
          <Menu menus={menus} trigger={input(member.role)} />
        ) : (
          input(member.role)
        )}
      </div>
