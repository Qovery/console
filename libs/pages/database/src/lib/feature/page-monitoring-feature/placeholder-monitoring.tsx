...
function PlaceholderDatabaseChart() {
  return (
    <Section className="h-full w-full rounded border border-neutral-250 p-4">
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Heading weight="medium">Database status</Heading>
            <Icon iconName="circle-info" iconStyle="regular" className="text-neutral-350" />
          </div>
          <div className="flex items-center gap-4">
            <span className="text-xs text-neutral-350">Available</span>
            <span className="text-xs text-neutral-350">Active connections</span>
            <Tooltip content="Show chart">
              <Button variant="outline" color="neutral" size="xs" iconOnly>
                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="none" viewBox="0 0 16 16">
                  <g fill="currentColor" fillRule="evenodd" clipPath="url(#clip0_25356_47547)" clipRule="evenodd">
...
export function PlaceholderMonitoring() {
  return (
    <div className="pointer-events-none grid h-full gap-3 md:grid-cols-1 xl:grid-cols-2">
      <PlaceholderDatabaseChart />
      <div className="flex h-full flex-col gap-3">
        <PlaceholderCard title="Database connections" description="active connections" icon="link" />

        <PlaceholderCard title="CPU utilization" description="database CPU usage" icon="microchip" />

        <PlaceholderCard title="Storage usage" description="% of allocated storage" />

        <PlaceholderCard title="IOPS performance" description="read/write operations" status="GREEN" />
      </div>
    </div>
  )
}
