...
export function CustomFilterFeature({ handleClearFilter }: CustomFilterFeatureProps) {
  ...
  const [queryParams] = useQueryParams(queryParamsValues)
  ...
  useEffect(() => {
    if (queryParams.fromTimestamp && queryParams.toTimestamp)
      setTimestamps([
        new Date(parseInt(queryParams.fromTimestamp, 10) * 1000),
        new Date(parseInt(queryParams.toTimestamp, 10) * 1000),
      ])
  }, [location])
