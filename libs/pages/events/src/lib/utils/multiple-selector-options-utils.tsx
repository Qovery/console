...
const SUB_TARGET_TYPES_BY_TARGET_TYPE: Map<string, string[]> = new Map<string, string[]>([
  ...
  [OrganizationEventTargetType.MEMBERS_AND_ROLES, [
    OrganizationEventSubTargetType.INVITATION,
    OrganizationEventSubTargetType.MEMBER_ROLE,
  ]],
...
export function buildSearchBarOptions(
  ...
  if (selectedOptions.length === 0) {
    return Object.values(OrganizationEventTargetType).map((targetType) => {
      ...
      return {
        ...
        label: label,
        description: 'Some description to test color',
      }
    })
  }
  console.log("buildSearchBarOptions", queryParams)
  ...
  const subTargetType = selectedOptions.find((option) => option.value.startsWith('subTargetType:'))
  ...
  // Handle other cases
    ...
    const isStandaloneType = targetTypeOption && !isServiceType(targetTypeOption)

    if (isStandaloneType && preFetchedTargets && preFetchedTargets.length > 0) {
      options.push(...preFetchedTargets)
    }
  }
...
export function fetchTargetsAsync(
  ...
  return async (): Promise<Option[]> => {
    ...
    console.log("Calling fetchTargetsAsync with targetFetchLevel=", targetFetchLevel)
