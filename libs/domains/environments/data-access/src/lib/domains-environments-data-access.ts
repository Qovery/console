...
export const environments = createQueryKeys('environments', {
  ...
  deploymentQueue: ({ environmentId }: { environmentId: string }) => ({
    queryKey: [environmentId],
    async queryFn() {
      const result = await environmentMainCallsApi.listDeploymentRequestByEnvironmentId(environmentId)
      return result.data.results
    },
  }),
...
export const mutations = {
  ...
  async uninstallEnvironment({ environmentId }: { environmentId: string }) {
    const result = await environmentActionApi.uninstallEnvironment(environmentId)
    return result.data
  },
