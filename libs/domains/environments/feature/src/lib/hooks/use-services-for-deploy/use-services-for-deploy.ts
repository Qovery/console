...
function mapServiceToDeployInfo(service: AnyService): ServiceForDeploy | undefined {
  ...
  if ('terraform_files_source' in service) {
    const tfGitRepo = service.terraform_files_source?.git?.git_repository
    return {
      id: service.id,
      name: service.name,
      icon_uri: service.icon_uri,
...
export function useServicesForDeploy({ environmentId }: UseServicesForDeployProps) {
  ...
  const servicesForDeploy = useMemo(
    () => services.map(mapServiceToDeployInfo).filter((service): service is ServiceForDeploy => Boolean(service)),
    [services]
  )
