...
function CurrentVersionBadge({ version, isOutdated }: { version?: ServiceVersionInfo; isOutdated: boolean }) {
  if (!version) {
    return <span className="text-neutral-350 text-xs">-</span>
  }

  if (version.type === 'commit') {
    return (
      <span className="border-neutral-250 inline-flex h-7 max-w-28 items-center overflow-hidden rounded border bg-white px-2 text-ssm font-normal text-neutral-400">
  ...
  return (
    <span className="border-neutral-250 inline-flex h-7 max-w-28 items-center overflow-hidden rounded border bg-white px-2 font-mono text-xs text-neutral-400">
  ...
  return (
    <Popover.Root>
      <Popover.Trigger>
        <button
          type="button"
          className="border-neutral-250 inline-flex h-7 w-fit max-w-28 items-center justify-between gap-2 overflow-hidden rounded border bg-white px-2 text-ssm font-normal text-neutral-400"
        ...
        >
          ...
          <Icon iconName="chevron-down" className="text-neutral-350 shrink-0" />
        </button>
      </Popover.Trigger>
      <Popover.Content
        className="border-neutral-250 max-h-[320px] min-w-[320px] max-w-[320px] overflow-y-auto overflow-x-hidden rounded-md border bg-white p-0"
        sideOffset={6}
      >
        <ul>
          {options.map((option) => {
            ...
            return (
              <li key={option.value} className="border-neutral-250 border-b last:border-b-0">
                <Popover.Close>
                  <button
                    type="button"
                  ...
                  >
                    ...
                    {option.bottomLabel ? (
                      <span className="text-neutral-350 block w-full max-w-full truncate whitespace-nowrap text-ssm font-normal">
                        {option.bottomLabel}
                      </span>
                    ) : null}
                  </button>
          ...
          {options.length === 0 && <li className="text-neutral-350 px-3 py-4 text-ssm">No versions available</li>}
        </ul>
      </Popover.Content>
    </Popover.Root>
  )
...
function ServiceCard({
  ...
  const isSelectable = !service.isDeletedBranch && !service.isSkipped
  ...
  return (
    <li
      ...
      title={service.isSkipped ? 'This service is skipped and cannot be deployed at environment level' : undefined}
      className={`border-neutral-250 flex h-14 items-center justify-between border-b px-4 last:border-b-0 ${
        service.isDeletedBranch || service.isSkipped ? 'bg-neutral-100' : isChecked ? 'bg-brand-50' : 'bg-white'
      }`}
    >
      <div
      ...
      >
        ...
        <span
          className={`min-w-0 text-sm font-medium ${
            service.isDeletedBranch || service.isSkipped ? 'text-neutral-300' : 'text-neutral-400'
          }`}
        >
      ...
      <div className="ml-4 flex items-center gap-2">
        {service.isDeletedBranch || service.isSkipped ? (
...
export function UpdateAllModal({ environment }: UpdateAllModalProps) {
  ...
  const { data: deploymentStages, isLoading: isDeploymentStagesLoading } = useListDeploymentStages({
    environmentId: environment.id,
  })
  ...
  const skippedServiceIds = useMemo(
    () =>
      new Set(
        deploymentStages?.flatMap(
          (stage) => stage.services?.filter(({ is_skipped }) => is_skipped).map(({ service_id }) => service_id) ?? []
        ) ?? []
      ),
    [deploymentStages]
  )
  ...
  useEffect(() => {
    ...
    const nextSelections: ServiceSelectionState = new Map(
      supportedServices.map((service) => {
        ...
        const isSkipped = skippedServiceIds.has(service.id)
        ...
        return [
          service.id,
          {
            ...
            isDeletedBranch,
            isSkipped,
            isSelected: isDeletedBranch || isSkipped ? false : computedIsOutdated,
            selectedVersion: isDeletedBranch || isSkipped ? undefined : computedDefaultVersion,
            latestOutdatedVersion: isDeletedBranch || isSkipped ? undefined : latestOutdatedVersion,
  ...
  const toggleService = useCallback((serviceId: string) => {
    setSelections((prev) => {
      ...
      if (!service || service.isDeletedBranch || service.isSkipped) return prev
  ...
  const selectAllOutdated = useCallback(() => {
    setSelections((prev) => {
      ...
      next.forEach((service, serviceId) => {
        if (!service.isOutdated || service.isSkipped) return
    ...
    return (
      <div className="w-full">
        <div className="mb-2 flex items-center justify-between">
          <p className="text-neutral-350 text-sm font-medium">{title}</p>
        ...
        <ul className="border-neutral-250 overflow-hidden rounded-md border">
  ...
  return (
    <div className="relative">
      <div className="max-h-[80vh] overflow-y-auto p-5 pb-[88px]">
        ...
        <p className="text-neutral-350 mb-4 text-sm font-normal dark:text-neutral-50">
          Select the service and choose a specific version to deploy on each of those
        </p>

        {isOutdatedServicesLoading || isServicesLoading || isDeploymentStagesLoading || isLatestVersionsLoading ? (
          <div className="py-6">
      ...
      <div className="dark:bg-neutral-550 absolute bottom-0 left-0 right-0 flex justify-end gap-3 bg-white px-5 py-4">
