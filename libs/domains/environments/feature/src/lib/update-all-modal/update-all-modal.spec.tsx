...
const mockUseListDeploymentStages = jest.fn(() => ({ data: [], isLoading: false, error: {} }))
...
jest.mock('../hooks/use-list-deployment-stages/use-list-deployment-stages', () => ({
  useListDeploymentStages: () => mockUseListDeploymentStages(),
}))
...
describe('UpdateAllModal', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks()
    mockUseListDeploymentStages.mockReturnValue({ data: [], isLoading: false, error: {} })
  ...
  it('should ignore toggle on unselectable row and toggle selected row with keyboard', async () => {
    ...
    if (!appRow) throw new Error('Expected App service row to be interactive')
    appRow.focus()
  ...
  it('should not select or allow interaction with skipped services', async () => {
    mockUseListDeploymentStages.mockReturnValueOnce({
      data: [{ services: [{ service_id: 'service-app', is_skipped: true }] }],
      isLoading: false,
      error: {},
    })

    const { userEvent } = renderWithProviders(<UpdateAllModal {...props} />)
    expect(await screen.findByText('Update 6 services')).toBeInTheDocument()

    const skippedRow = screen.getByText('App service').closest('[role="button"]')
    expect(skippedRow).toHaveAttribute('aria-disabled', 'true')
    expect(document.querySelector('input[name="service-app"]')).toBeNull()

    await userEvent.click(screen.getByText('App service'))
    expect(screen.getByText('Update 6 services')).toBeInTheDocument()
  })
