import clsx from 'clsx'
import { type ClusterLogs, ClusterLogsStepEnum } from 'qovery-typescript-axios'
import { memo } from 'react'
import { LogsType } from '@qovery/shared/enums'
import { CopyToClipboardButtonIcon } from '@qovery/shared/ui'
import { dateDifference, dateFullFormat, dateUTCString } from '@qovery/shared/util-dates'
import {
  LOG_COPY_EXCLUDE_DATA_ATTRIBUTE,
  LOG_MESSAGE_DATA_ATTRIBUTE,
  copySelectedLogMessages,
  twMerge,
} from '@qovery/shared/util-js'

export interface ClusterLogRowProps {
  data: ClusterLogs
  index: number
  firstDate?: Date
}

interface RenderedClusterLogRowState {
  index: number
  firstDateTimestamp?: number
  logTimestamp?: string
  step?: ClusterLogs['step']
  type?: ClusterLogs['type']
  safeMessage?: string
  errorMessage?: string
}

function getRenderedClusterLogRowState({ data, index, firstDate }: ClusterLogRowProps): RenderedClusterLogRowState {
  return {
    index,
    firstDateTimestamp: firstDate?.getTime(),
    logTimestamp: data.timestamp,
    step: data.step,
    type: data.type,
    safeMessage: data.message?.safe_message,
    errorMessage: data.error?.user_log_message,
  }
}

function ClusterLogRowRoot({ index, data, firstDate }: ClusterLogRowProps) {
  const type = data.type
  const success = data.step === ClusterLogsStepEnum.CREATED
  const realError =
    data.step === ClusterLogsStepEnum.DELETE_ERROR ||
    data.step === ClusterLogsStepEnum.PAUSE_ERROR ||
    data.step === ClusterLogsStepEnum.CREATE_ERROR
  const message = type === LogsType.ERROR ? data.error?.user_log_message : data.message?.safe_message

  const indexClassName = clsx({
    'group-hover:bg-surface-negative-componentHover bg-surface-negative-component text-neutral':
      type === LogsType.ERROR && realError,
    'group-hover:bg-surface-positive-componentHover bg-surface-positive-component text-neutral':
      success && !(type === LogsType.ERROR && realError),
    'bg-surface-neutral-subtle text-neutral-subtle group-hover:bg-surface-neutral-componentHover':
      !success && !(type === LogsType.ERROR && realError),
  })

  const colorsCellClassName = clsx({
    'text-negative': type === LogsType.ERROR,
    'text-warning': type === LogsType.WARNING,
    'text-positive': success && type !== LogsType.ERROR && type !== LogsType.WARNING,
  })

  return (
    <div
      onCopy={copySelectedLogMessages}
      className={twMerge(
        clsx(
          'group flex min-h-6 justify-between font-code text-xs [contain-intrinsic-size:0_24px] [content-visibility:auto] hover:bg-surface-neutral-subtle',
          {
            'bg-surface-positive-subtle text-positive': success,
            'bg-surface-negative-component text-neutralInvert': realError,
          }
        )
      )}
    >
      <div className="flex">
        <div data-testid="index" className={indexClassName}>
          <div className="h-6 w-10 min-w-[40px] select-none px-2 py-1 text-right">{index + 1}</div>
        </div>
        <div
          data-testid="cell-date"
          className={twMerge('px-2 py-1 selection:bg-transparent selection:text-neutral-subtle', colorsCellClassName)}
        >
          <span {...{ [LOG_COPY_EXCLUDE_DATA_ATTRIBUTE]: 'true' }}>
            {firstDate && dateDifference(new Date(data.timestamp as string), firstDate)}
          </span>
        </div>
        <div data-testid="cell-msg" className={twMerge('w-11/12 py-1', colorsCellClassName)}>
          <span className="font-bold">{data.step} - </span>
          <span className="whitespace-pre-wrap" {...{ [LOG_MESSAGE_DATA_ATTRIBUTE]: 'true' }}>
            {message}
          </span>
        </div>
      </div>
      <div
        className={twMerge(
          'flex whitespace-nowrap px-2 py-1 selection:bg-transparent selection:text-neutral-subtle',
          colorsCellClassName
        )}
      >
        <CopyToClipboardButtonIcon
          className="mr-4 select-none text-neutral opacity-0 group-hover:opacity-100"
          content={message as string}
        />
        {data.timestamp && (
          <span title={dateUTCString(data.timestamp)} {...{ [LOG_COPY_EXCLUDE_DATA_ATTRIBUTE]: 'true' }}>
            {dateFullFormat(data.timestamp, undefined, 'dd MMM, HH:mm:ss.SS')}
          </span>
        )}
      </div>
    </div>
  )
}

function areClusterLogRowPropsEqual(previousProps: ClusterLogRowProps, nextProps: ClusterLogRowProps) {
  const previousState = getRenderedClusterLogRowState(previousProps)
  const nextState = getRenderedClusterLogRowState(nextProps)

  return (
    previousState.index === nextState.index &&
    previousState.firstDateTimestamp === nextState.firstDateTimestamp &&
    previousState.logTimestamp === nextState.logTimestamp &&
    previousState.step === nextState.step &&
    previousState.type === nextState.type &&
    previousState.safeMessage === nextState.safeMessage &&
    previousState.errorMessage === nextState.errorMessage
  )
}

export const ClusterLogRow = memo(ClusterLogRowRoot, areClusterLogRowPropsEqual)

export default ClusterLogRow
