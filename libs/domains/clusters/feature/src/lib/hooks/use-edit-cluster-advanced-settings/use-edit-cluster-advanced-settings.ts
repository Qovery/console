...
export function useEditClusterAdvancedSettings() {
  ...
  return useMutation(mutations.editAdvancedSettings, {
    onSuccess(_, { organizationId, clusterId }) {
      queryClient.invalidateQueries({
        queryKey: queries.clusters.advancedSettings({ organizationId, clusterId }).queryKey,
      })
    },
    meta: {
      notifyOnSuccess(_: unknown, variables: unknown) {
        const { organizationId, clusterId } = variables as Parameters<typeof mutations.editAdvancedSettings>[0]
        return {
...
export default useEditClusterAdvancedSettings
