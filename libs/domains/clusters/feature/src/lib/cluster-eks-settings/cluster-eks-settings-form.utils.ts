...
export interface ClusterEksSettingsFormData extends ClusterResourcesData {
  ...
  provider?: keyof typeof GitProviderEnum | null
  git_token_id?: GitTokenResponse['id'] | null
  git_token_name?: GitTokenResponse['name'] | null
  root_path?: string
}
...
export const getInfrastructureChartsParametersWithEksAnywhereGit = (
  ...
  const gitTokenId =
    data.git_token_id === null
      ? ''
      : data.git_token_id ?? currentEksAnywhereParameters?.git_repository?.git_token_id ?? ''
