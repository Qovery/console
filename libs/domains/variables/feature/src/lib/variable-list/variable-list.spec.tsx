...
describe('VariableList', () => {
  ...
  it('should display all variables', () => {
    ...
    const builtInSection = screen.getByRole('heading', { name: /built-in variables/i }).closest('section')
  ...
  it('should filter variables by name', async () => {
    ...
    const customSection = screen.getByRole('heading', { name: /custom variables/i }).closest('section')
    await userEvent.click(within(customSection as HTMLElement).getByRole('button', { name: /value/i }))
    await userEvent.click(screen.getByRole('menuitem', { name: /private/i }))
    const rows = within(customSection as HTMLElement).getAllByRole('row')
    expect(rows).toHaveLength(2)
  })
  it('should have doppler actions', async () => {
    const { userEvent } = renderWithProviders(<VariableList {...variableListProps} />)
    const row = screen.getByText('DOPPLER_PROJECT').closest('tr')
    expect(row).toBeTruthy()
    await userEvent.click(within(row as HTMLElement).getByRole('button', { name: /actions/i }))
    const menuItem = await screen.findByRole('menuitem', { name: /edit in doppler/i })
    expect(menuItem).toBeInTheDocument()
  })
  it('should have built-in actions', async () => {
    const { userEvent } = renderWithProviders(<VariableList {...variableListProps} />)
    const builtInSection = screen.getByRole('heading', { name: /built-in variables/i }).closest('section')
    expect(builtInSection).toBeTruthy()
    const row = within(builtInSection as HTMLElement)
      .getAllByText('QOVERY_OUTPUT_JOB_Z33962E52_DB_USERNAME')[0]
      .closest('tr')
    expect(row).toBeTruthy()
    await userEvent.click(within(row as HTMLElement).getByRole('button', { name: /actions/i }))
  ...
  it('should search variables by name', async () => {
    const { userEvent } = renderWithProviders(<VariableList {...variableListProps} />)
    const customSection = screen.getByRole('heading', { name: /custom variables/i }).closest('section')
    await userEvent.type(within(customSection as HTMLElement).getByRole('textbox'), 'FOO')
    const rows = within(customSection as HTMLElement).getAllByRole('row')
    expect(rows).toHaveLength(2)
  })
  it('should search variables by name with aliases', async () => {
    const { userEvent } = renderWithProviders(<VariableList {...variableListProps} />)
    const customSection = screen.getByRole('heading', { name: /custom variables/i }).closest('section')
    await userEvent.type(within(customSection as HTMLElement).getByRole('textbox'), 'BAR')
    const rows = within(customSection as HTMLElement).getAllByRole('row')
