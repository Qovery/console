      ...
      {!isLoading && chartData.length > 0 && (
        <Chart.Legend
          ...
          content={(props) => {
            ...
            const nginxSeries = seriesNames.filter((name) => {
              try {
                const metric = JSON.parse(name)
                return metric.source === 'nginx'
              } catch {
                return false
              }
            })

            const envoySeries = seriesNames.filter((name) => {
              try {
                const metric = JSON.parse(name)
                return metric.source === 'envoy'
              } catch {
                return false
              }
            })
