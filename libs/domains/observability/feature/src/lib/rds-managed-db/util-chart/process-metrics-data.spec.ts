...
describe('processMetricsData', () => {
  const mockMetricData: RdsMetricData = {
    metric: {
    ...
    const multiSeriesData = {
      data: {
        result: [
          ...
          {
            ...mockMetricData,
            metric: { ...mockMetricData.metric, dimension_DBInstanceIdentifier: 'second-db-instance' },
  ...
  it('should use custom getSeriesName function', () => {
    processMetricsData(
      mockMetricsResponse,
      timeSeriesMap,
      (series) => series.metric.dimension_DBInstanceIdentifier || 'unknown',
      (value) => parseFloat(value),
      false
    )

    const dataPoint = timeSeriesMap.get(1704067200000)
    expect(dataPoint?.['test-db-instance']).toBe(50.5)
  })
