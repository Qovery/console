...
import {
  ...
  type AgenticWorkflowAutomationOutput,
...
type SheetView = 'provider' | 'mcp' | 'skills' | 'automation' | null
...
const AUTOMATION_APPLICATIONS = ['Linear', 'Slack', 'Jira', 'GitHub', 'Grafana', 'HubSpot', 'Figma']
const AUTOMATION_APPLICATION_EVENTS = ['Qovery agent mention', 'New issue created', 'Comment added']
const AUTOMATION_APPLICATION_CHANNELS = ['#eng-alerts', '#devops', '#product']
const AUTOMATION_FREQUENCIES = ['Daily', 'Weekly', 'Monthly']
const AUTOMATION_TIMES = ['8:00am', '9:00am', '12:00pm', '5:00pm']
const AUTOMATION_TIMEZONES = ['UTC', 'Europe/Paris', 'America/New_York']
const AUTOMATION_DAYS = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday']
const AUTOMATION_MONTH_DAYS = Array.from({ length: 28 }, (_, index) => {
  const day = index + 1
...
interface TextAreaFieldProps extends Omit<ComponentPropsWithoutRef<'textarea'>, 'className'> {
  label: string
}
...
function TextAreaField({ label, ...props }: TextAreaFieldProps) {
  return (
    <Field label={label}>
      <textarea
        {...props}
        className="min-h-[116px] resize-y rounded-md border border-neutral bg-surface-neutral-component p-3 text-sm leading-5 text-neutral outline-none placeholder:text-neutral-disabled focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-strong"
      />
    </Field>
  )
}
...
function SelectControl({ ariaLabel, disabled, onChange, options, value }: SelectFieldProps) {
...
function SelectField({ label, ...props }: SelectFieldProps & { label: string }) {
  return (
    <Field label={label}>
      <SelectControl {...props} />
    </Field>
...
export function AgentCreationActions({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <DropdownMenu.Root>
        <DropdownMenu.Trigger asChild>
          <Button type="button" variant="outline" color="neutral" size="sm" iconOnly aria-label="Agent actions">
        ...
        <DropdownMenu.Content align="end" className="z-dropdown">
          <DropdownMenu.Item icon={<Icon iconName="copy" />} onSelect={onCloneAgent}>
            Clone
          </DropdownMenu.Item>
          <DropdownMenu.Separator />
          <DropdownMenu.Item color="red" icon={<Icon iconName="trash" />} onSelect={onDeleteAgent}>
            Delete agent
          </DropdownMenu.Item>
        </DropdownMenu.Content>
      </DropdownMenu.Root>
...
function AddRowButtonContent({ children }: { children: ReactNode }) {
  return (
    <>
...
function AddRowButton({ children, onClick }: { children: ReactNode; onClick: () => void }) {
  return (
    <Button type="button" variant="plain" color="neutral" size="sm" className="text-neutral-subtle" onClick={onClick}>
      <AddRowButtonContent>{children}</AddRowButtonContent>
    </Button>
  )
}
...
function LinkedBadge({ children, icon, onClick }: { children: ReactNode; icon: ReactNode; onClick: () => void }) {
  return (
    <Button
      ...
      color="neutral"
      size="sm"
      className="rounded-md p-1.5 retina:border-[0.5px]"
      onClick={onClick}
    >
      {icon}
      {children}
...
function AutomationBadgeIcon({ automation }: { automation: AgenticWorkflowAutomation }) {
  if (automation.triggers.length === 1 && automation.triggers[0]) {
    return <AutomationTriggerIcon trigger={automation.triggers[0]} />
  }

  return (
    <span className="flex h-4 w-4 items-center justify-center rounded-full bg-surface-neutral-component font-code text-xs font-bold text-neutral">
...
function AgentSettingRow({
  ...
  const labelNode = <span className="w-24 shrink-0 text-sm font-medium text-neutral-subtle">{label}</span>
  const contentClassName = 'flex min-h-9 min-w-0 flex-1 flex-wrap items-center gap-2 rounded-md px-1 py-1'
    ...
    return (
      <div className="flex min-h-11 items-center gap-4 py-1">
        ...
        <button
          ...
          className={twMerge(
            contentClassName,
            'w-full text-left outline-0 transition-colors hover:bg-surface-neutral-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-strong'
          )}
          onClick={emptyAction.onClick}
        >
          <span className="inline-flex h-7 items-center gap-x-1 rounded-md border border-neutral bg-surface-neutral px-1.5 text-ssm font-medium text-neutral retina:border-[0.5px]">
            <AddRowButtonContent>{emptyAction.label}</AddRowButtonContent>
          </span>
        </button>
      </div>
    )
  ...
  return (
    <div className="flex min-h-11 items-center gap-4 py-1">
      {labelNode}
      <div className={twMerge(contentClassName, 'transition-colors hover:bg-surface-neutral-subtle')}>{children}</div>
    </div>
  )
}
...
function McpSheet({ onAddMcp, onClose }: { onAddMcp: () => void; onClose: () => void }) {
  ...
  return (
    <OverlaySheet onClose={onClose}>
      <SheetHeader title="Manage MCP" className="px-5 pb-5 pt-4" onClose={onClose} />
      <div className="flex flex-1 flex-col overflow-auto px-4">
        ...
        {filteredServers.length ? (
            ...
            {filteredServers.map((server) => {
              ...
              return (
                <div
                  ...
                  aria-checked={linked}
                  tabIndex={0}
                  className="flex h-10 w-full cursor-pointer select-none items-center justify-start gap-3 rounded pl-1 pr-3 text-left transition-colors hover:bg-surface-neutral-subtle focus-visible:bg-surface-neutral-subtle focus-visible:outline focus-visible:outline-2 focus-visible:outline-brand-strong active:bg-surface-neutral-component"
                ...
                >
                  ...
                  <Icon
                    ...
                    className={linked ? 'text-sm text-positive' : 'text-sm text-neutral-subtle'}
                  />
                </div>
              )
            })}
          ...
          <div className="mt-3 flex flex-1 flex-col items-center justify-center gap-3 px-1 pb-24 text-center">
...
function AutomationApplicationIcon({ application }: { application: string }) {
  ...
  if (normalized.includes('jira')) return <Icon iconName="jira" iconStyle="brands" />
  if (normalized.includes('hubspot')) return <Icon iconName="hubspot" iconStyle="brands" />
  return <Icon iconName="cube" />
}

function AutomationTriggerIcon({ trigger }: { trigger: AgenticWorkflowAutomationTrigger }) {
  if (trigger.type === 'application') {
    return <AutomationApplicationIcon application={trigger.application ?? ''} />
  }

  if (trigger.type === 'schedule') return <Icon iconName="calendar-day" iconStyle="regular" />

  return <Icon iconName="webhook" />
}

function AutomationOutputIcon({ output }: { output: AgenticWorkflowAutomationOutput }) {
  if (output.type === 'application') {
    return <AutomationApplicationIcon application={output.application ?? ''} />
  }

  return <Icon iconName="webhook" />
}

function getAutomationTriggerTitle(trigger: AgenticWorkflowAutomationTrigger) {
  if (trigger.type === 'application') return trigger.application ?? 'Application'
  if (trigger.type === 'schedule') return 'Schedule'
  return 'Webhook trigger'
}

function getAutomationTriggerDescription(trigger: AgenticWorkflowAutomationTrigger) {
  if (trigger.type === 'application') {
    return `${trigger.event ?? 'Event'} on ${trigger.channel ?? 'selected channel'}`
  }

  if (trigger.type === 'schedule') {
    if (trigger.frequency === 'Daily') {
      return `Daily at ${trigger.time ?? '8:00am'} ${trigger.timezone ?? 'UTC'}`
    }

    if (trigger.frequency === 'Monthly') {
      return `Monthly on the ${trigger.dayOfMonth ?? '1st'} at ${trigger.time ?? '8:00am'} ${trigger.timezone ?? 'UTC'}`
    }

    return `Every ${(trigger.dayOfWeek ?? 'Monday').toLowerCase()} at ${trigger.time ?? '8:00am'} ${
...
function getAutomationOutputTitle(output: AgenticWorkflowAutomationOutput) {
  if (output.type === 'application') return output.application ?? 'Application output'
  return 'Webhook output'
}

function getAutomationOutputDescription(output: AgenticWorkflowAutomationOutput) {
  if (output.type === 'application') return output.destination ?? 'Selected destination'
  ...
  return (
    <div className="flex h-11 items-center justify-between gap-3 rounded-md border border-neutral bg-surface-neutral-component px-3">
      <span className="flex min-w-0 flex-1 items-center gap-2">
        <span className="shrink-0 text-sm text-neutral">{icon}</span>
...
function ApplicationTriggerModal({
  onSave,
  setOpen,
  trigger,
}: {
  ...
  const [search, setSearch] = useState('')
  const [application, setApplication] = useState<string | undefined>(trigger?.application)
  const [event, setEvent] = useState(trigger?.event ?? AUTOMATION_APPLICATION_EVENTS[0])
  const [channel, setChannel] = useState(trigger?.channel ?? AUTOMATION_APPLICATION_CHANNELS[0])
  const filteredApplications = AUTOMATION_APPLICATIONS.filter((item) =>
      ...
      <div className="flex flex-col gap-5 p-5">
        <div className="flex items-start justify-between gap-4 pr-8">
          <h2 className="text-xl font-medium leading-7 text-neutral">
            {trigger ? 'Edit application trigger' : 'Add application trigger'}
          </h2>
        </div>
        <div className="flex h-11 items-center justify-between rounded-md border border-neutral bg-surface-neutral-component px-3">
          <span className="flex items-center gap-2 text-sm font-medium text-neutral">
            <AutomationApplicationIcon application={application} />
            {application}
          </span>
          <Button type="button" variant="plain" color="neutral" size="xs" onClick={() => setApplication(undefined)}>
            Edit
          </Button>
        </div>
        <SelectField label="Event" options={AUTOMATION_APPLICATION_EVENTS} value={event} onChange={setEvent} />
        <SelectField label="Channel" options={AUTOMATION_APPLICATION_CHANNELS} value={channel} onChange={setChannel} />
        <div className="flex justify-end gap-2">
          <Button type="button" variant="plain" color="neutral" size="md" onClick={() => setOpen?.(false)}>
            Cancel
          </Button>
          <Button type="button" size="md" onClick={saveTrigger}>
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex flex-col gap-1">
          {filteredApplications.map((item) => (
            <div key={item} className="flex h-10 items-center justify-between gap-3">
              <span className="flex items-center gap-3">
                <span className="flex h-8 w-8 items-center justify-center rounded border border-neutral bg-surface-neutral-subtle text-sm text-neutral">
                  <AutomationApplicationIcon application={item} />
                </span>
                <span className="text-ssm font-medium text-neutral">{item}</span>
              </span>
              <Button type="button" variant="outline" color="neutral" size="xs" onClick={() => setApplication(item)}>
                Connect
              </Button>
            </div>
          ))}
...
function ScheduledTriggerModal({
  ...
  const [frequency, setFrequency] = useState(trigger?.frequency ?? 'Weekly')
  const [time, setTime] = useState(trigger?.time ?? '8:00am')
  const [timezone, setTimezone] = useState(trigger?.timezone ?? 'UTC')
  const [dayOfWeek, setDayOfWeek] = useState(trigger?.dayOfWeek ?? 'Monday')
  const [dayOfMonth, setDayOfMonth] = useState(trigger?.dayOfMonth ?? '1st')

  return (
    <div className="flex flex-col gap-5 p-5">
      <div className="flex items-start justify-between gap-4 pr-8">
        <h2 className="text-xl font-medium leading-7 text-neutral">
          {trigger ? 'Edit scheduled trigger' : 'Add scheduled trigger'}
        </h2>
      </div>
      <SelectField label="Frequency" options={AUTOMATION_FREQUENCIES} value={frequency} onChange={setFrequency} />
      <div className="flex w-full flex-col gap-1 text-sm text-neutral-subtle">
        <span>Time</span>
        <div className="grid grid-cols-[1fr_120px] items-end gap-2">
          <SelectControl ariaLabel="Time" options={AUTOMATION_TIMES} value={time} onChange={setTime} />
          <SelectControl ariaLabel="Timezone" options={AUTOMATION_TIMEZONES} value={timezone} onChange={setTimezone} />
        </div>
      </div>
      {frequency === 'Weekly' ? (
        <SelectField label="Day of the week" options={AUTOMATION_DAYS} value={dayOfWeek} onChange={setDayOfWeek} />
      ) : null}
      {frequency === 'Monthly' ? (
        <SelectField
          label="Day of the month"
          options={AUTOMATION_MONTH_DAYS}
          value={dayOfMonth}
...
function WebhookTriggerModal({
  ...
  const webhookUrl = trigger?.webhookUrl ?? 'https://qovery.com/api/automations/webhooks/whk_gCKtonKG3W3pjaprXkMc4'
  const signingSecret = trigger?.signingSecret ?? 'whsec_Z8xvpsH3Qp0cKc3nD9hW'

  return (
    <div className="flex flex-col gap-5 p-5">
      <div className="flex flex-col gap-1 pr-8">
        <h2 className="text-xl font-medium leading-7 text-neutral">
          {trigger ? 'Edit webhook trigger' : 'Add webhook trigger'}
        </h2>
        <p className="text-sm leading-5 text-neutral-subtle">
          POST a JSON payload to a unique URL. Authenticated by HMAC.
        </p>
      </div>
      <Field label="Webhook URL">
        <div className="flex h-10 items-center gap-2 rounded-md border border-neutral bg-surface-neutral-component px-3 text-sm text-neutral">
          <span className="min-w-0 flex-1 truncate">{webhookUrl}</span>
          <Button type="button" variant="plain" color="neutral" size="xs" iconOnly aria-label="Copy webhook URL">
            <Icon iconName="copy" />
          </Button>
        </div>
      </Field>
      <Field label="Signing secret">
        <div className="flex h-10 items-center gap-2 rounded-md border border-neutral bg-surface-neutral-component px-3 text-sm text-neutral">
          <span className="min-w-0 flex-1 truncate">{showSecret ? signingSecret : '••••••••••••••••••••••••••'}</span>
          <Button
            type="button"
            variant="plain"
            color="neutral"
          ...
          >
            <Icon iconName={showSecret ? 'eye-slash' : 'eye'} />
          </Button>
          <Button type="button" variant="plain" color="neutral" size="xs" iconOnly aria-label="Copy signing secret">
            <Icon iconName="copy" />
          </Button>
        </div>
      </Field>
      <div className="flex justify-end gap-2">
        ...
        <Button
          ...
          size="md"
          onClick={() => {
            onSave({ id: trigger?.id ?? crypto.randomUUID(), type: 'webhook', webhookUrl, signingSecret })
            setOpen?.(false)
          }}
        >
          {trigger ? 'Save trigger' : 'Confirm'}
...
function ApplicationOutputModal({
  onSave,
  output,
  setOpen,
}: {
  onSave: (output: AgenticWorkflowAutomationOutput) => void
  output?: AgenticWorkflowAutomationOutput
  setOpen?: (open: boolean) => void
}) {
  const [search, setSearch] = useState('')
  const [application, setApplication] = useState<string | undefined>(output?.application)
  const [destination, setDestination] = useState(output?.destination ?? '#eng-alerts')
  const filteredApplications = AUTOMATION_APPLICATIONS.filter((item) =>
    item.toLowerCase().includes(search.toLowerCase())
  )
  ...
  const saveOutput = () => {
    ...
    onSave({ id: output?.id ?? crypto.randomUUID(), type: 'application', application, destination })
    setOpen?.(false)
  }
  ...
  if (application) {
    return (
      <div className="flex flex-col gap-5 p-5">
        <div className="flex flex-col gap-1 pr-8">
          <h2 className="text-xl font-medium leading-7 text-neutral">
            {output ? 'Edit application output' : 'Add application output'}
          </h2>
          <p className="text-sm leading-5 text-neutral-subtle">Send the agent result to a connected application.</p>
        </div>
        <div className="flex h-11 items-center justify-between rounded-md border border-neutral bg-surface-neutral-component px-3">
          <span className="flex items-center gap-2 text-sm font-medium text-neutral">
            <AutomationApplicationIcon application={application} />
        ...
        <SelectField
          label="Destination"
          options={AUTOMATION_APPLICATION_CHANNELS}
          value={destination}
        ...
        <div className="flex justify-end gap-2">
          ...
          <Button type="button" size="md" onClick={saveOutput}>
...
function WebhookOutputModal({
  ...
  const [webhookUrl, setWebhookUrl] = useState(output?.webhookUrl ?? 'https://example.com/agent-result')
  const [signingSecret, setSigningSecret] = useState(output?.signingSecret ?? '')

  return (
    <div className="flex flex-col gap-5 p-5">
      <div className="flex flex-col gap-1 pr-8">
        <h2 className="text-xl font-medium leading-7 text-neutral">
          {output ? 'Edit webhook output' : 'Add webhook output'}
        </h2>
        <p className="text-sm leading-5 text-neutral-subtle">POST the final result to an external endpoint.</p>
      </div>
      <TextField
        label="Webhook URL"
        value={webhookUrl}
      ...
      <div className="flex justify-end gap-2">
        ...
        <Button
          ...
          onClick={() => {
            onSave({ id: output?.id ?? crypto.randomUUID(), type: 'webhook', webhookUrl, signingSecret })
            setOpen?.(false)
          }}
        >
          {output ? 'Save output' : 'Add output'}
...
function AutomationSheet({ automation, onClose }: { automation?: AgenticWorkflowAutomation; onClose: () => void }) {
  const { form } = useAgenticWorkflowCreateContext()
  ...
  const saveOutput = (output: AgenticWorkflowAutomationOutput) => {
    setDraft((current) => ({
      ...current,
      outputs: current.outputs.some((item) => item.id === output.id)
        ? current.outputs.map((item) => (item.id === output.id ? output : item))
        : [...current.outputs, output],
    }))
  }

  const closeAutomationModal = (open: boolean) => {
    if (!open) closeModal()
  }

  const openTriggerModal = (type: AgenticWorkflowAutomationTriggerType, triggerId?: string) => {
    const trigger = triggerId ? draft.triggers.find((item) => item.id === triggerId) : undefined

    openModal({
      content:
        type === 'application' ? (
          <ApplicationTriggerModal trigger={trigger} onSave={saveTrigger} setOpen={closeAutomationModal} />
        ) : type === 'schedule' ? (
          <ScheduledTriggerModal trigger={trigger} onSave={saveTrigger} setOpen={closeAutomationModal} />
        ) : (
          <WebhookTriggerModal trigger={trigger} onSave={saveTrigger} setOpen={closeAutomationModal} />
        ),
      options: {
        width: 488,
      },
  ...
  const openOutputModal = (type: AgenticWorkflowAutomationOutputType, outputId?: string) => {
    const output = outputId ? draft.outputs.find((item) => item.id === outputId) : undefined

    openModal({
      content:
        type === 'application' ? (
          <ApplicationOutputModal output={output} onSave={saveOutput} setOpen={closeAutomationModal} />
        ) : (
          <WebhookOutputModal output={output} onSave={saveOutput} setOpen={closeAutomationModal} />
        ),
      options: {
        width: 488,
      },
  ...
  const saveAutomation = () => {
    ...
    const nextAutomation = { ...draft, name: draft.name.trim(), prompt: draft.prompt.trim() }
    form.setValue(
      'automations',
      automation
        ? values.automations.map((item) => (item.id === automation.id ? nextAutomation : item))
        : [...values.automations, nextAutomation],
      { shouldDirty: true }
    )
    onClose()
  }
  ...
  return (
    <OverlaySheet onClose={onClose}>
      <SheetHeader title="Add automation" onClose={onClose} />
      <div className="flex flex-1 flex-col gap-4 overflow-auto px-5 pb-5">
        ...
        <AutomationSection
          title="Trigger"
          description="An event that starts the automation. Could be a schedule, a webhook, or an action performed in a connected tool."
          action={
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <Button type="button" variant="outline" color="neutral" size="sm">
              ...
              <DropdownMenu.Content align="end" className="z-dropdown w-56">
                <DropdownMenu.Item icon={<Icon iconName="cube" />} onSelect={() => openTriggerModal('application')}>
                  From application
                </DropdownMenu.Item>
                <DropdownMenu.Item
                  icon={<Icon iconName="calendar-day" iconStyle="regular" />}
                  onSelect={() => openTriggerModal('schedule')}
                >
                  On a schedule
                </DropdownMenu.Item>
                <DropdownMenu.Item icon={<Icon iconName="webhook" />} onSelect={() => openTriggerModal('webhook')}>
                  From a webhook
                </DropdownMenu.Item>
              </DropdownMenu.Content>
            </DropdownMenu.Root>
          ...
          {draft.triggers.length ? (
            <div className="flex flex-col gap-2">
              {draft.triggers.map((trigger) => (
                <AutomationItemCard
                  key={trigger.id}
                  icon={<AutomationTriggerIcon trigger={trigger} />}
                  title={getAutomationTriggerTitle(trigger)}
                  onEdit={() => openTriggerModal(trigger.type, trigger.id)}
                  onRemove={() =>
                    setDraft((current) => ({
                      ...current,
                ...
                >
                  {getAutomationTriggerDescription(trigger)}
                </AutomationItemCard>
              ))}
            </div>
          ) : null}
        ...
        <AutomationSection
          title="Task"
          description="What you want the agent to do when this automation runs. Type {{ to insert data from the trigger."
        >
          <TextAreaField
        ...
        <AutomationSection
          title="Output"
          description="What the agent does with its result at the end of its task. Optional."
          action={
            <DropdownMenu.Root>
              <DropdownMenu.Trigger asChild>
                <Button type="button" variant="outline" color="neutral" size="sm">
              ...
              <DropdownMenu.Content align="end" className="z-dropdown w-52">
                <DropdownMenu.Item icon={<Icon iconName="cube" />} onSelect={() => openOutputModal('application')}>
                  To application
                </DropdownMenu.Item>
                <DropdownMenu.Item icon={<Icon iconName="webhook" />} onSelect={() => openOutputModal('webhook')}>
                  To custom webhook
                </DropdownMenu.Item>
              </DropdownMenu.Content>
            </DropdownMenu.Root>
          ...
          {draft.outputs.length ? (
            <div className="flex flex-col gap-2">
              {draft.outputs.map((output) => (
                <AutomationItemCard
                  key={output.id}
                  icon={<AutomationOutputIcon output={output} />}
                  title={getAutomationOutputTitle(output)}
                  onEdit={() => openOutputModal(output.type, output.id)}
                  onRemove={() =>
                    setDraft((current) => ({
                      ...current,
                ...
                >
                  {getAutomationOutputDescription(output)}
                </AutomationItemCard>
              ))}
            </div>
          ) : null}
      ...
      <div className="border-t border-neutral p-5">
        <Button type="button" className="w-full justify-center" size="lg" disabled={!canSave} onClick={saveAutomation}>
          Save
        </Button>
      </div>
    </OverlaySheet>
          ...
          <DropdownMenu.Root>
            <DropdownMenu.Trigger asChild>
              <Button
                ...
                color="neutral"
                size="xs"
                className="gap-1.5 px-0 text-sm text-neutral"
              >
...
export function DockerSettings({ onAddRaw }: { onAddRaw: () => void }) {
  const { form } = useAgenticWorkflowCreateContext()
  const dockerFragment = form.watch('dockerFragment')
  const fileInputRef = useRef<HTMLInputElement>(null)
  const [dockerFragmentTitle, setDockerFragmentTitle] = useState('Dockerfile fragment')
  ...
  const onFileChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    void readFileAsText(file)
      .then((content) => {
        setDockerFragmentTitle(file.name)
        form.setValue('dockerFragment', content, { shouldDirty: true })
      })
      .catch(() => undefined)
      .finally(() => {
        input.value = ''
    ...
    <>
      ...
      {dockerFragment ? (
        <div className="flex items-center gap-3 rounded-lg border border-neutral bg-surface-neutral p-3">
          ...
          <span className="min-w-0 flex-1 truncate text-ssm leading-[18px] text-neutral">{dockerFragmentTitle}</span>
          <Button
            type="button"
            variant="outline"
            color="neutral"
            ...
            onClick={() => {
              setDockerFragmentTitle('Dockerfile fragment')
              form.setValue('dockerFragment', '', { shouldDirty: true })
            }}
          >
            <Icon iconName="trash-can" />
          </Button>
        ...
        <div className="flex gap-2">
          <input
            ...
            aria-label="Dockerfile fragment file"
            className="hidden"
...
export function AgenticWorkflowConfiguration() {
  ...
  const editingAutomation = useMemo(
    () => values.automations.find((automation) => automation.id === editingAutomationId),
    [editingAutomationId, values.automations]
  )
  ...
  const cloneAgent = () => {
    ...
    const name = currentValues.name.trim() || 'New agent'

    form.reset({ ...currentValues, name: `${name}-clone` }, { keepDirty: true })
  }
  ...
  return (
    <div className="absolute inset-0 z-10 flex min-h-[640px] min-w-[960px] overflow-hidden bg-background">
        ...
        <main className="flex-1 overflow-auto border-b border-neutral px-20 py-10">
          <div className="flex max-w-[920px] flex-col">
            <div className="flex flex-col items-start gap-3">
              <img src={agenticWorkflowIcon.icon} alt="" className="h-12 w-12" />
              <InlineTitle
                value={values.name}
            ...
            <section className="border-t border-neutral py-3">
              <AgentSettingRow
                label="Provider"
                emptyAction={
                  values.provider ? undefined : { label: 'Add provider', onClick: () => setProviderModalOpen(true) }
                }
              >
                {values.provider ? (
                  <>
              ...
              <AgentSettingRow
                label="MCP"
                emptyAction={
                  linkedMcpServers.length ? undefined : { label: 'Add MCP', onClick: () => setActiveSheet('mcp') }
                }
              >
              ...
              <AgentSettingRow
                label="Skills"
                emptyAction={
                  values.skills.length ? undefined : { label: 'Add skill', onClick: () => setActiveSheet('skills') }
                }
              >
                {values.skills.length ? (
                  <>
              ...
              <AgentSettingRow
                label="Automations"
                emptyAction={
                  values.automations.length
                    ? undefined
                    : { label: 'Add automations', onClick: () => openAutomationSheet() }
                }
              >
                {values.automations.length ? (
                  <>
                    {values.automations.map((automation) => (
                      <LinkedBadge
                        key={automation.id}
                        icon={<AutomationBadgeIcon automation={automation} />}
                        onClick={() => openAutomationSheet(automation.id)}
                      >
                        {automation.name}
                      </LinkedBadge>
                    ))}
                    <AddRowButton onClick={() => openAutomationSheet()}>Add automations</AddRowButton>
                  </>
                ) : null}
              </AgentSettingRow>
            </section>
      ...
      {activeSheet === 'automation' ? (
        <AutomationSheet
          key={editingAutomationId ?? 'new-automation'}
