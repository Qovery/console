...
function mapSecretForForm(
  secret: AddSecretModalSubmitData,
  sourceById: Map<string, { tableLabel: string; icon: 'aws' | 'gcp' }>,
  scope: 'APPLICATION' | 'CONTAINER'
): ExternalSecretData {
  const source = sourceById.get(secret.secretManagerAccessId)
  return {
    name: secret.name,
    ...
    secretManagerAccessId: secret.secretManagerAccessId,
    source: source?.tableLabel ?? null,
...
export function ApplicationContainerStepVariables({ onBack, onSubmit }: ApplicationContainerStepVariablesProps) {
  ...
  const openSecretModal = ({ isFile, index }: { isFile: boolean; index?: number }) => {
    ...
    const defaultSource =
      secretSources.find((source) => source.value === currentSecret?.secretManagerAccessId) ??
      secretSources.find((source) => source.tableLabel === currentSecret?.source) ??
      secretSources[0]
    ...
    openModal({
      content: (
        <AddSecretModal
          ...
          onClose={closeModal}
          onSubmit={(secret) => {
            const mapped = mapSecretForForm(secret, sourceById, serviceScope)
            if (typeof index === 'number') {
