...
export interface UseUninstallAllServicesProps {
  organizationId: string
  projectId: string
}

export function useUninstallAllServices() {
  const queryClient = useQueryClient()
  const navigate = useNavigate()

  return useMutation(mutations.uninstallAllServices, {
    ...
    meta: {
      notifyOnSuccess(_: unknown, variables: unknown) {
        ...
        } = variables as Parameters<typeof mutations.uninstallAllServices>[0]
        return {
...
export default useUninstallAllServices
