...
function formatBlueprintProvider(provider: string) {
...
function getBlueprintRepositoryUrlFromReadme(readme: string | undefined, repositoryName: string) {
  if (!readme) return undefined

  const [organizationName, repositorySlug] = repositoryName.split('/')
  const repositoryUrlPattern = new RegExp(
    `https://github\\.com/${organizationName}/${repositorySlug}(?:[\\w\\-/?.#=&%]*)?`,
    'i'
  )

  return readme.match(repositoryUrlPattern)?.[0]
}
...
function BlueprintDetailsPanel({
  blueprint,
  organizationId,
  open,
  onOpenChange,
  ...
  const {
    ...
    isLoading,
    isError,
  } = useBlueprintCatalogServiceReadme({
    organizationId,
  ...
  const provider = formatBlueprintProvider(blueprint.provider)
  const repositoryName = getBlueprintRepositoryName(blueprint)
  const repositoryUrl = getBlueprintRepositoryUrlFromReadme(readme, repositoryName)
          ...
          <div className="flex-1 overflow-auto px-6 pb-24 pt-6">
            <div className="mb-8 flex flex-col gap-5">
              ...
              <div className="flex flex-wrap items-center gap-2">
                {repositoryUrl ? (
                  <ExternalLink href={repositoryUrl} withIcon={false} aria-label={`Open ${repositoryName} repository`}>
                    {repositoryBadge}
                  </ExternalLink>
                ) : (
                  repositoryBadge
